...
for trials in range(100):                                                  # <<<<<<<<<<<<
    ...
    for n in range(n_sets):
        weight_name        = "100x100x100_25_0.000001_4m_0.2_[" + str(0 + n + 1) +  "]_weight_list.npy"   # <<<<<<<<<<<<
        slope_name         = "100x100x100_25_0.000001_4m_0.2_[" + str(0 + n + 1) +  "]_slope_list.npy"    # <<<<<<<<<<<<
    ...
    for t in range(10000):                                                    # <<<<<<<<<<<<
        ...
        state_value                              = np.atleast_2d(inverse_sigmoid( np.concatenate((state_0, state_1)) ) ) # <<<<<<<<<<<<
        action_value                             = np.atleast_2d((np.random.random((1, 3*70)) - 0.5) * 0.00 - 3.5 )      # <<<<<<<<<<<<
        ...
        state_and_acton_value_resistor[:, 200:]  = 1                                                                     # <<<<<<<<<<<<
        reward                                   = np.ones(100)                                                          # <<<<<<<<<<<<
        ...
        action_value = state_and_acton_value[:, 200:]                                                                     # <<<<<<<<<<<<
